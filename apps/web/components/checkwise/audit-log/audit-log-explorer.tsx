...
/**
 * AuditLogExplorer — the cross-tenant audit-log browser (filters +
 * paginated table + before/after diff).
 *
 * Extracted from ``/platform/audit-log`` so it can render inside EITHER
 * staff shell without duplicating ~770 lines. The backend
 * (``GET /admin/audit-log``) authorizes BOTH ``platform_admin`` and
 * ``operations_admin``; the page that mounts this picks the shell and
 * supplies the entry gate. This component is shell-agnostic — it owns
 * no title/description and no role gate (audit F2/F13).
 */
