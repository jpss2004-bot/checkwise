...
// Type-only — the motion/react RUNTIME (Reorder + drag controls) lives in the
// lazily-loaded EditableBlockList so it never ships to read-only/print/Story
// consumers of this Canvas.
...
// Drag-reorder stack (motion/react) loaded only when the editable branch
// renders — read-only/print/StoryView never pull the motion runtime.
const EditableBlockList = dynamic(
  () =>
    import("@/components/checkwise/reports/editable-block-list").then(
      (m) => m.EditableBlockList,
    ),
  { ssr: false },
);
