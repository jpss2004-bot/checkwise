...
_ADMIN_MONTHLY_OPERATIONAL = ReportPreset(
    ...
    recommended_prompt=(
        ...
        "compliance_overview (cifras clave + cumplimiento por proveedor) "
        "y un bloque compliance_by_institution (barras por SAT / IMSS / "
        "INFONAVIT / STPS-REPSE). Luego: resumen ejecutivo con métricas, "
        "KPI strip con porcentaje de cumplimiento, total de proveedores y "
        "proveedores en riesgo, matriz cruzada de proveedores ordenada por "
        "risk_score, y una recomendación final de 5 acciones priorizadas "
...
_CLIENT_MONTHLY_EXECUTIVE = ReportPreset(
    ...
    recommended_prompt=(
        ...
        #
        # 2026-06-05 — de-dup pass, then partial reversal. We briefly
        # dropped ``compliance_radar`` on the theory it was a third
        # redundant worst-first vendor ranking. That was stale: the
        # radar's ranked-vendor list was already removed on 2026-06-03,
        # so today the block is a semáforo donut + a 6-month approval
        # sparkline — and that sparkline is the ONLY trend view in the
        # whole report (the fetcher always computes history_6mo, see
        # data_fetchers.fetch_compliance_radar). Restored it with
        # ``include_history=true``. The only remaining overlap is the
        # donut vs the overview's semáforo KPI — a distinct hero viz,
        # worth keeping for the trend it carries.
        ...
        "cumplimiento por proveedor. Es la portada de cifras.\n"
        "2. ``compliance_by_institution`` (sin configuración) — barras "
        ...
        "``include_history=true`` — dona del semáforo del portafolio "
        "+ tendencia de aprobación de los últimos 6 meses. Es la "
        "única tendencia del reporte.\n"
...
_PROVIDER_CURRENT_STATE = ReportPreset(
    ...
    recommended_prompt=(
        ...
        "un bloque compliance_by_institution que muestre, en barras, "
        "cuántos de tus documentos por institución (SAT / IMSS / "
        "INFONAVIT / STPS-REPSE) están al día, en proceso o en riesgo. "
        "Sigue con un bloque attention_list sin filtros (todos los documentos "
