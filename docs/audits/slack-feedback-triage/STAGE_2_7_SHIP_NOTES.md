...
**Backend ([backend/app/core/compliance_catalog.py](../../../backend/app/core/compliance_catalog.py)):**
...
**API ([backend/app/api/v1/portal.py](../../../backend/app/api/v1/portal.py)):**
...
- [frontend/lib/api/portal.ts](../../../frontend/lib/api/portal.ts) — added the three fields on `CalendarItem`.
- [frontend/components/checkwise/portal/expediente-card.tsx](../../../frontend/components/checkwise/portal/expediente-card.tsx) — extracted `DocumentGuidanceDisclosure` to a public exported surface that takes `{anatomy, where_to_obtain, common_errors, summary_label?}`. Backward-compatible: the existing in-page call site uses the new prop signature.
- [frontend/app/portal/calendar/page.tsx](../../../frontend/app/portal/calendar/page.tsx) — `CalendarEntry` carries the three new fields; the drawer renders the shared disclosure with a periodicity-aware label "Acerca de este comprobante".

**Tests ([backend/tests/test_portal_canonical_enrichment.py](../../../backend/tests/test_portal_canonical_enrichment.py)):**
...
**Backend ([backend/app/services/correction_request_service.py](../../../backend/app/services/correction_request_service.py)) — new service module:**
...
**Endpoint ([backend/app/api/v1/portal.py](../../../backend/app/api/v1/portal.py)):**
...
- [frontend/lib/api/corrections.ts](../../../frontend/lib/api/corrections.ts) — new real API client. Exports `TIER_B_FIELDS`, `TIER_B_FIELD_LABEL_ES`, `submitCorrectionRequest`. Mirrors the existing portal API auth resolution (`Authorization: Bearer` + `credentials: "include"`). The legacy `frontend/lib/mock/corrections.ts` stays in the tree — it still backs the inline editable profile on the same page — but the correction-request flow no longer touches it.
- [frontend/components/checkwise/workspace/correction-request-form.tsx](../../../frontend/components/checkwise/workspace/correction-request-form.tsx) — rewritten. Field selector is restricted to the three Tier B fields. Submission calls the real API client. Always-on info alert pointing providers to `soporte@checkwise.mx` for non-Tier-B changes.
- [frontend/app/portal/entra-a-tu-espacio/page.tsx](../../../frontend/app/portal/entra-a-tu-espacio/page.tsx) — mounted the form as a new "Solicitar corrección de un dato de contacto" section below the existing inline-editable profile form.

**Tests ([backend/tests/test_portal_correction_requests.py](../../../backend/tests/test_portal_correction_requests.py)):**
...
**Backend ([backend/app/services/submission_service.py](../../../backend/app/services/submission_service.py)):**
...
**Schemas ([backend/app/schemas/submissions.py](../../../backend/app/schemas/submissions.py)):**
...
**Frontend ([frontend/components/checkwise/intake-wizard.tsx](../../../frontend/components/checkwise/intake-wizard.tsx)):**
...
**Tests ([backend/tests/test_portal_multi_file_upload.py](../../../backend/tests/test_portal_multi_file_upload.py)):**
...
- The legacy `frontend/lib/mock/corrections.ts` still backs the inline-editable profile on `/portal/entra-a-tu-espacio`. Migrating that surface to a real backend endpoint is its own track (not Tier B — those are first_name / last_name / phone / job_title / contact_preference).
