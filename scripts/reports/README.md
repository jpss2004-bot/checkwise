...
| `build_sample_sandbox.py` | Rebuild the off-repo `_reference/sample-docs/` testing sandbox from a `Banco Docs Sample-*.zip`. Normalizes filenames, builds by-vendor + by-institution indexes, writes `manifest.json` and the sandbox README. | `python3 scripts/reports/build_sample_sandbox.py <zip-path>` |

All four are optional. The product runs without them.
